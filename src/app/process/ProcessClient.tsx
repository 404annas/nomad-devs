  ...
  return (
    <main className="bg-white text-black selection:bg-gray-100 overflow-x-hidden">
      ...
      <section className="py-10 px-6 bg-[#F3F4F6]">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center gap-4 mb-16">
            <h2 className="text-xs lg:text-base font-bold uppercase tracking-wide">Our Approach</h2>
            <div className="h-px flex-1 bg-gray-300" />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
            {steps.map((step, i) => (
              <motion.div key={i} {...fadeInUp} transition={{ delay: i * 0.1 }}>
                <span className="text-3xl font-bold text-gray-400 mb-4 block">{step.num}</span>
                <h3 className="text-sm lg:text-lg font-bold uppercase mb-4">{step.title}</h3>
                <p className="text-sm lg:text-base text-gray-500 leading-normal">{step.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
