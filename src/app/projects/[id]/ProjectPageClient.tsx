...
const ProjectPage = () => {
  ...
  const projectName = (project.title || project.id.replace(/-/g, " ")).trim();
    ...
    <div className="min-h-screen bg-white text-black">
      ...
      <div className="w-full max-w-[1400px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-4 min-h-[550px] px-4 py-4">
        ...
        <div className="relative w-full h-[650px]">
          <Image
            src={project.mainPic}
            alt={`${projectName} design and build project featured image`}
            fill
          ...
          <div className="flex flex-col gap-6 sticky top-24">
            {sideImages.map((img, idx) => (
              <div key={idx} className="relative w-full h-[400px] md:h-[600px] cursor-pointer hover:opacity-95 transition-opacity duration-300 shadow-sm" onClick={() => openLightbox(idx)}>
                <Image src={img} alt={`${projectName} project detail image ${idx + 1}`} fill className="object-cover" />
              </div>
            ))}
          </div>
        </div>
      ...
      {remainingImages.length > 0 && (
        <div className="max-w-[1400px] mx-auto px-4 grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
          {remainingImages.map((img, idx) => (
            ...
            >
              <Image src={img} alt={`${projectName} project gallery image ${idx + 2}`} fill className="object-cover" />
            </div>
          ))}
        </div>
      )}
          ...
          >
            ...
            <div className="relative w-full max-w-6xl h-[80vh]">
              <Image
                ...
                alt={`${projectName} project preview image ${currentImageIndex + 1}`}
                fill
